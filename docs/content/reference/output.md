...
describes what users get. Pick a format with `-o`, or let commons choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
commons <command> -o table   # aligned columns for reading
commons <command> -o jsonl   # one JSON object per line, for piping
commons <command> -o json    # a single JSON array
commons <command> -o csv     # spreadsheet friendly
commons <command> -o tsv     # tab-separated
commons <command> -o url     # just the URL column
commons <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
commons <command> --fields id,title,url
```
...
```bash
commons <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
commons <command>            # a table, because this is a terminal
commons <command> | wc -l    # JSONL, because this is a pipe
```
